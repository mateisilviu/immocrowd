...
class _ManagePropertiesScreenState extends State<ManagePropertiesScreen> {
  ...
  Future<void> _saveForm() async {
    ...
    try {
      List<String> firebaseRefs = List.empty(growable: true);
      ...
      if (_imageFileList != null) {
        for (var i = 0; i < _imageFileList!.length; i++) {
          var ref = FirebaseStorage.instance
              .ref()
              .child(Property.COLLECTION_NAME)
              .child('images')
              .child(_imageFileList![i].name);
          await ref.putData(await _imageFileList![i].readAsBytes());
          ref.getDownloadURL().then((url) => {
                firebaseRefs.add(url),
                newProperty = Property.updatePhoto(firebaseRefs, addedProperty),
                Provider.of<PropertiesProvider>(context, listen: false)
                    .updateProperty(newProperty)
              });
        }
      }
      print("length : ${firebaseRefs.length}");
      await showDialog(
        context: context,
